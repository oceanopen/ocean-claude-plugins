...
**状态与上下文分离**：子任务清单、状态、进度以数据库为唯一真相源（`ocean-harness-cli mcp call issue_child_list` / tracker 看板），上下文文件不记录任何状态——同一数据只留一份，杜绝双写漂移。CLAUDE.md 只是需求上下文的本地快照。
...
- **不写入子任务清单、状态、进度等任何随执行变化的数据**——agent-dev 从 DB（`ocean-harness-cli mcp call issue_child_list`）获取清单与状态，凭返回的子任务 id 经 CLI 回写，不经本文件
- 「原始需求（存档）」段一经写入永不修改
